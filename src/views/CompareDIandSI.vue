<template>
  ...
  <h2 v-else class="compare__no-data">Для выбранного кадра отсутствуют соответствующие формы СИ</h2>
</template>
...
const comparisonTable: any = reactive({
  ...
  // Строки таблицы
  rows: computed(() =>
    comparison.data?.result.map(({ param, di, si, diff }: ComparisonRecord) => {
